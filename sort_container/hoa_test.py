...
def sort_bay(tiers):
    ...
    for tier in tiers:
        print("trai", tier[0])
        print("tong trai", calculas_total_weight(tier[0]))
        print("phai", tier[1])
        print("tong phai", calculas_total_weight(tier[1]))
        print()

        matrix_balance = [[0 for x in range(len(tier[0]))] for x in range(len(tier[1]))]
        print(matrix_balance)
        # bias_basic = w_r_left - w_r_right
        # for i in range(len(tier[0])):
        #     matrix_balance[0][i] = sys.maxsize
        # for r in range(1, len(tier[1])):
        #     matrix_balance[r][r - 1] = bias_basic
        #
        # while matrix_balance[0][0] != matrix_balance[1][0]:
        #     for r in range(1, len(tier[1])):
        #         index = 0
        #         for c in range(r, len(tier[0])):
        #             matrix_balance[r][c] = matrix_balance[r][c - 1] - 2 * (tier[0][c].weight - tier[1][c].weight)
...
def main():
    ...
    f_data_harbour = "data_har_auto.txt"
